...
proc determinantal_resolve(matrix M, int sizeOfMinor){
	...
	if(defined(centerlist)){
		if(size(centerlist)>1){
			cent = calculate_center();	//TODO later delete superfluent entries in centerlist
		}else{
			stat = "transformation";
			datalist[3][1] = stat;
		}
	...
	for(i = 2; i<= size(BlowUpTree);i++){			
		...
		if(defined(BO) or defined(centerlist)){
			if(defined(BO)){
				ideal cent = calculate_center();			//BO could be deleted if center equals 1; No center than
			...
			if(defined(centerlist)){
				if(size(centerlist) > 1){
					ideal cent = calculate_center();	//TODO later delete superfluent entries in centerlist
					export(cent);
				}else{
					stat = "transformation";
					datalist[3][1] = stat;
		...
		if(!defined(cent)){				//new status have to start; no center already choosen
			...
			if(stat == "transformation"){
				if(defined(centerlist)){			//senseful?
					kill(centerlist);
				}
...
"
{
	...
	if(EqualityOfIdeal(Center,0) == 1){	//Transformation status
		...
		//benötigt keine for-schleife mehr. wird direkt in generateCovering getan
	}else{	
...
//----------------------------------------------------------------------
//TODO!!!
proc generateOneCovering(list datalist){
	int i,j,k;
	ring old_r = basering;
	matrix M = datalist[2];
...
//TODO: list to ideal:		normcrossideal = ideal(N[1]);
//							complementideal = ideal(N[2]);	
		if(defined(new_r)){kill(r_new);}
		...
		setring(new_r);
		list datalist;
		datalist[1] = preimage(old_r,f,datalist[1]);
		datalist[2] = preimage(old_r,f,datalist[2]);
		datalist[3] = preimage(old_r,f,datalist[3]);
		datalist[4] = preimage(old_r,f,datalist[4]);
		datalist[5] = preimage(old_r,f,datalist[5]);
