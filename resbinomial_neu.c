...
"{
	...
	while(descend_finished(center, btmp, CoeffBOList[size(CoeffBOList)]) == 0){	
		...
		if(casenumber == 2){
			...
			BOtmp[3] = b[size(b)] * BOcontrol[3];	//superfluent?
		}
		...
		if(orderdecreased == 0 and (size(b_old) >= counter)){ 
			...
			if(b[counter] < b_old[counter]){						
				...
				orderdecreased = 1;
				BO7new[counter] = size(BOtmp[4]);		//ggf aendern. vorher war hier -1
			}else{print("fehler2");BO7new[counter] = BO7old[counter];}
		} else{
			BO7new[counter] = 0;
		}
...
proc isSmooth(ideal J)		//TODO via slocus?
